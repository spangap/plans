...
- **The packets inside a burst are split-framed as usual.** The peer counts
...
the thing changed (ANNOUNCE1 and the announce channel, `SUPE.channel plan`,
`announce_bundling`), because a missed occurrence is exactly the drift phase 0
exists to prevent.

- **`s.lora.<n>.afa` is the channel plan number; `SUPE.channel plan` does not exist.** The
  ...
  channel plan 0, on with `afa` 1 is channel plan 1. That costs no migration, keeps the
  ...
  say "0 is not a channel plan — it means no agility", while SUPE.md §3/§4 make
  channel plan 0 a working channel plan, never renumbered. The reuse survives because
  channel plan 0 has no channel plan, so `regimeChans(0)` yielding nothing stays
...
- **ANNOUNCE1 and the announce channel are dropped.** SUPE.md §16 already had
  ...
  the main channel in every channel plan, as channel plan 0 always did. That deletes a frame
  type, a channel role and a per-announcement deafness window for every listener
  in earshot, and makes channel plans 0 and 1 announce identically; what it costs is
...
- **SUPE.md §14.4 states a requirement, not a data structure.** What a channel plan
...
- **A channel switch is bounded to complete inside the daemon's existing timeout
  rate step** — SUPE.md §12's explicit choice, taken provisionally now so the
...
| §14 channel table and index space | `LORA_CH_HAIL` (0), `LORA_CH_MAX` (10), `RegimeChan`, channel plan 1's nine channels | `lora.cpp:93` |
| §14.4 per-channel airtime **telemetry** | `Rolling1h txAir[LORA_CH_MAX]` — per channel, already fed. **Telemetry only: it cannot enforce the rate step** — see §6 and phase 8 | `lora.cpp:927`, `rolling.h` |
...
| §15 clamp to configured power | `apClamp()` — already clamps to chip range *and* `tx_power`, which is SUPE.md §15's limit rule | §15 |
...
- The tag set of SUPE.md §5: a flat array of `{ uint8_t tag[3]; uint32_t expiry;
  ...
  figure is a limit, not a target, and this is PSRAM-allocated alongside
  `NeiState`.
- **The hold list of SUPE.md §6**: tags overheard in other senders' STARTs, each
  ...
  concurrently announced channel switch in earshot — consulted by the transmit gate (§5).
...
- Detour state machine: phase, chosen channel and step, the START's own SHA-256
  prefix, both deadlines, the saved calling config to return to.
...
- **Path-loss pairs of SUPE.md §10**: a level measured here together with the
  ...
  configuration (calling, and the step last used). These start flowing in
  phase 5 — ANNOUNCE2's power byte against our own reading of the frame is the
  first pair, at the calling configuration — with the probe's joining in
...
**Compiled, not state: the channel plan tables.** Everything SUPE.md §3 lists as "a
constant of the channel plan", keyed by channel plan and version — channels, rate table, sync
words, frame layouts and lengths, duration encodings, limits, limits, retune
guard, expiry. The expiry is a CALENDAR DATE compiled in (§3), moved by hand as
the protocol version is revised — a date rather than an offset from each build, so that
every node on a channel retires the protocol version at the same moment however far apart
...
- **resolve(channel plan, version, calling config, step) → configuration, or "no such
  step"**: the bitrate-ordered rate table of SUPE.md §14.3, channel plan 0's restriction to
  the calling bandwidth, both families' limits — no SF5 on SX127x, SF6 barred
...
- **the top step a pair can use**, from both capability bytes and the calling
...
- **encode and decode every frame**, with the length test driven by the same
  tables: a frame's permitted lengths enumerable from channel plan, version and type
...
- **both deadlines** — sender-waits-for-HERE and peer-waits-for-MANIFEST — from
  channel plan constants plus the step, and the duration (20 ms) and length (5 ms)
  byte encodings with their limits. The retune turnaround in that formula is
  a named constant with a deliberately generous provisional value: deadlines
  must end up channel plan constants both sides derive identically, so phase 7's
...
- **expiry**: past the compiled date, frames naming that channel plan/version are
...
1. **Never dropped, never channel switched** — announces and path requests (`neiParse`
...
3. **Detour or probe** — the tag resolves to a neighbour whose ANNOUNCE2 has
   been heard: channel switch if a step is available to the pair, probe first if the
   peer is stale (§11's five-minute gate).
4. **Plain** — everything else, exactly as an interface with SUPE off.

The channel switch state machine then owns the radio the way a radio check does — its
own phase gate at the top of `drainOneOutbound`, the same pattern as `rc.phase`
— so a channel switch, a radio check and an announce tick can never interleave.
...
codec, deadline arithmetic, with host-side tests covering the rate table edge cases
(SX127x pairs, slow-calling networks, low-data-rate boundaries, expiry). Have
the tests emit golden frame vectors — the exact on-air bytes per frame type,
channel plan and length form — as an artefact; every later phase that injects a frame
...
prints the set with its expiries, plus the compiled channel plan expiry (§3); that is
...
**Phase 5 — ANNOUNCE2 on the main channel, every channel plan.** A node announces its
...
carries a calling-configuration measurement before it is ever offered
...
back, then the traffic plainly. This is the whole scheduled exchange mechanism — sense,
...
`csmaClear()` — channel plan 0 inherits the calling channel's rules, and the
...
the probe as the channel switch state machine with its retune states unreachable:
...
`esp_timer` path or channel plan 1's compliance claim gets revisited — decide where
the loop is simplest. Independently useful: it stops full packets going to
absent peers, which is measurable on its own.

**Phase 7 — the channel switch, channel plan 0.** Add the retune: START, both ends to a
higher spreading factor on the same frequency, HERE, MANIFEST, burst, home.
...
step — channel plan 0's escalation moves the step only, having no channels to move —
...
§4, and the chosen value lands in SUPE.md §14 as a channel plan constant — both
...
this is the first phase in which a burst can vanish, and the decaying counter
...
channel plan 0's second step from an SF7 network is SF5 and `0x21`'s behaviour there
is unconfirmed; and the bench is SX126x-class parts or a slower calling
configuration — an SX127x pair calling at SF7 has zero channel plan-0 steps
(SUPE.md §14.3), and a null result there is the rate table working, not a bug.

**Phase 8 — channel plan 1.** Frequency agility, the nine channels, the escalation
walk — channel changes with every offer, no reuse within a transaction, floor at
step 0, plus the 100 ms minimum frequency-reuse gap after a channel switch. Before the
ring is shaped, settle SUPE.md §16's one-rate step-or-two question — whether
calling airtime counts against the same rate step as channel switch airtime — since it
decides whether channel 0 carries a ring at all. The
enforcement ring of §3: `Rolling1h`'s six 10-minute buckets cannot defend
"100 s in any 3600 s" — a node can spend the rate step late in a bucket, have it
age out, and spend it again, approaching twice the cap inside a true hour — so
channel plan 1 gets its own finer per-channel ring with a precomputed verdict, and
...
- [x] `SUPE.sender_ident` marked deferred in §4 — v1 receives the 10-byte
      form, never sends it
- [x] `SUPE.channel plan` replaced by `s.lora.<n>.afa` in §3 and §4
- [x] sender candidate-channel sensing removed from §11; §16 item struck
- [x] §14.4 restated as a property, not a data structure
- [x] §12 bounded-channel switch choice recorded, marked provisional until phase 7
...
- [x] channel plan tables for channel plans 0 and 1, version 0, expiry stamped as the
      calendar date compiled in (SUPE.md §3), moved by hand as the protocol version
      is revised
- [x] `resolve(channel plan, version, calling, step)` with family limits (no SF5 on
      SX127x, SF6 barred for family 1) and the low-data-rate rule
- [x] sync word from the resulting configuration (`own` / `0x67` / `0x21`)
- [x] top step for a pair from both capability bytes plus calling config
...
- [x] duration (20 ms) and length (5 ms) encodings with limits
- [x] expiry: past the date, nothing sent, frames naming it discarded
- [x] host tests over the rate table edge cases; golden frame vectors emitted as
      an artefact
...
- [x] `lora <n> supe`: tag set with expiries, plus the compiled channel plan expiry
...
**Phase 5 — ANNOUNCE2**
- [x] announce tick on the existing beat's timer mechanism; interval from
...
**Phase 6 — the probe**
- [x] channel switch state machine, probe states only; retune states unreachable
...
**Phase 7 — the channel switch, channel plan 0**
...
- [x] burst transmit loop bounded by the announced duration; peer ends on
      count or length
...
- [x] channel switch airtime to `txAir[ch]`, calling duty view untouched
...
- [x] debug lines: offer out (channel, step, and what chose them), HERE heard,
      MANIFEST sent, burst done, returned home, deadline expired

**Phase 8 — channel plan 1**
- [x] one-rate step-or-two decided before the ring is shaped
...
- [x] 100 ms frequency-reuse gap after a channel switch
...
- [x] debug lines: escalation move, channel verdict flip, rate step refusal
...
- [x] debug lines: offset moved and why, floor set, power chosen per channel switch
...
- The test for the split: at debug a channel switch reads as a short story — offer,
  HERE, MANIFEST, burst, home — with no frame dumps between the lines; at
  verbose the same story is interleaved with every frame. A line that would
  fire per packet inside a burst belongs at verbose with the frames.
...
new channel argument threads both: records show channel switch frames on the channel
...
when the first channel switch records appear — only the built app was in this tree.
...
- **The task tick is 10 ms.** SUPE.md §14.2's 5 ms dead time between a clear
  ...
  pair moves to the `esp_timer` path the radio check already uses for its time slot
  transmits, or channel plan 1's compliance claim needs revisiting. Settle this in
  phase 6, where the loop is simplest.
- **Transmission is synchronous** (`radio.transmit()` runs inline on the task,
  `INTERNALS.md` §6), so a burst is a loop that blocks the task for its
  duration. That is acceptable — nothing else can use the radio anyway — but the
  ITS poll does not run, so bound the burst and do not let a channel switch outlive its
  announced duration.
- **`splitPending` gates all transmission.** A channel switch must not begin mid-split.
  Take the same early-out `drainOneOutbound()` uses.
- **The radio must never be stranded off the calling channel.** `rssiSweepAgile`
...
- **Detour airtime must not be credited to the calling channel.** SUPE.md §8
  ...
  calling-channel transmissions only. Get this wrong and the channel switch stops
...
- **`txAir` measures; it does not enforce.** `Rolling1h` is six 10-minute
  ...
  rate steps. Regime 1's cap gets the separate ring of §3; do not bolt enforcement
  onto the telemetry.
...
- `sender_ident` — **sending only.** The flag changes the START's length and
  ...
  governs nothing.
- Any reverse traffic on a channel switch. The modem has no per-peer queue and SUPE.md
  §17 already says so.
...
- **Retune turnaround** (phase 7), on the timer path rather than the task path.
  ...
  is worth.
- **The §12 confirmation** (phase 7): does a bounded channel switch fit inside Channel's
  retransmit rate step at the measured turnaround? §1's provisional answer is yes;
...
- **The first offer's step** (phase 7) — what a sender offers a peer with
  nothing filed beyond phase 5's announce pair, which covers the calling
  configuration only. SUPE.md §10 implies a slow first channel switch without stating
  a policy; whether that pair plus the margin table justifies opening higher
  is the decision.
- **One duty rate step or two** (phase 8) — whether calling and unicast airtime
  share a rate step (SUPE.md §16). It shapes the enforcement ring, deciding
...
| one duty rate step or two | phase 8 — blocking, above |
...
| §3 — channel plans, versions, lengths, sync words, quantisation | phase 2 |
...
| §14 — channel plans | tables in phase 2; enforcement ring in phase 8 |
